...
class HomeAssistantWebSocketClient:
    ...
    async def _handle_motion_event(
        ...
        for i, area_config in enumerate(sensor_config.areas):
            ...
            if (
                ...
                and entity_id not in area_config.trigger_entities
            ):
                continue
    ...
    async def _send_via_reach_or_fallback(
        ...
        Args:
            ...
            results: List of StepResult/True/None per area
            switch_id: Switch that triggered this (for bounce feedback)
            direction: "up" or "down" (for bounce feedback)
            bounce_type: "step" or "bright" (for bounce feedback)
        """
    ...
    def _get_feedback_group_for_area(
        ...
        filt_norm = best_filter.replace(" ", "_").lower()
        group_entity = group_candidates.get(f"zha_group_{filt_norm}_color")
        if not group_entity:
            group_entity = group_candidates.get("zha_group_color")
        if not group_entity:
    ...
    async def _feedback_cue(
        ...
        Args:
            switch_id: The switch IEEE address
            cue_type: "bounce" (limit hit), "freeze" (toggle freeze), "reach" (scope change)
    ...
    async def _deliver_filtered(
            ...
            for filter_name, lights_by_cap in filter_groups.items():
                ...
                if log_periodic:
                    ...
                    logger.info(
                        ...
                        f"target={p1_target}, bri={p1_bri}%, {kelvin}K"
                    )
        ...
        for filter_name, lights_by_cap in filter_groups.items():
            ...
            if skip_filters and filt_norm_check in skip_filters:
                if log_periodic:
                    logger.info(
                        f"Purpose '{filter_name}': skipped (handled by reach group)"
                    )
                continue
            ...
            if should_off:
                ...
                if off_entities:
                    ...
                    if zha_off_color:
                        if log_periodic:
                            logger.info(
                                f"Purpose OFF ({filter_name}): {zha_off_color}, brightness below {off_threshold}%"
                            )
                        tasks.append(
                            asyncio.create_task(
                                self.call_service(
                    ...
                    if zha_off_ct:
                        if log_periodic:
                            logger.info(
                                f"Purpose OFF ({filter_name}): {zha_off_ct}, brightness below {off_threshold}%"
                            )
                        tasks.append(
                            asyncio.create_task(
                                self.call_service(
                    ...
                    if non_group:
                        if log_periodic:
                            logger.info(
                                f"Purpose OFF ({filter_name}): {len(non_group)} individual lights"
                            )
                        tasks.append(
                            asyncio.create_task(
                                self.call_service(
            ...
            if lights_by_cap["color"]:
                ...
                if zha_group:
                    ...
                    if log_periodic:
                        logger.info(
                            f"Purpose update ({filter_name} color ZHA): {zha_group}, brightness={comp_brightness}%, {kelvin}K, transition={transition}s"
                        )
                    target_tasks.append(
                        asyncio.create_task(
                            self.call_service(
                    ...
                    if log_periodic:
                        logger.info(
                            f"Purpose update ({filter_name} color): {len(lights_by_cap['color'])} lights, brightness={comp_brightness}%, {kelvin}K, transition={transition}s"
                        )
                    target_tasks.append(
                        asyncio.create_task(
                            self.call_service(
            ...
            if lights_by_cap["ct"]:
                ...
                if zha_group:
                    ...
                    if log_periodic:
                        logger.info(
                            f"Purpose update ({filter_name} CT ZHA): {zha_group}, brightness={comp_brightness}%, transition={transition}s"
                        )
                    target_tasks.append(
                        asyncio.create_task(
                            self.call_service(
                    ...
                    if log_periodic:
                        logger.info(
                            f"Purpose update ({filter_name} CT): {len(lights_by_cap['ct'])} lights, brightness={comp_brightness}%, transition={transition}s"
                        )
                    target_tasks.append(
                        asyncio.create_task(
                            self.call_service(
            ...
            if lights_by_cap["brightness"]:
                ...
                if log_periodic:
                    logger.info(
                        f"Purpose update ({filter_name} brightness): {len(lights_by_cap['brightness'])} lights, brightness={comp_brightness}%, transition={transition}s"
                    )
                target_tasks.append(
                    asyncio.create_task(
                        self.call_service(
            ...
            if lights_by_cap["onoff"]:
                if log_periodic:
                    logger.info(
                        f"Purpose update ({filter_name} on/off): {len(lights_by_cap['onoff'])} lights"
                    )
                target_tasks.append(
                    asyncio.create_task(
                        self.call_service(
            ...
            try:
                for filter_name, lights_by_cap in filter_groups.items():
                    ...
                    try:
                        ...
                        logger.error(
                            f"2-step phase2 error for {area_id}/{filter_name}: {e}"
                        )
            except Exception as e:
            ...
            if sensor.trigger_entities:
                for te in sensor.trigger_entities:
                    if te not in self.motion_sensor_ids:
                        ...
                        logger.debug(
                            f"  Registered trigger entity {te} -> device:{dev_id}"
                        )
    ...
    async def handle_service_event(self, service, area_id, **kwargs):
        ...
        if service == "lights_on":
            ...
            await self.handle_config_refresh()
        else:
            logger.warning(
                        ...
                        try:
                            for area_id in circadian_areas:
                                ...
                                if _zone and _zone not in _solar_cached_zones:
                                    ...
                                    try:
                                        _zcfg = Config.from_dict(
                                            glozone.get_effective_config_for_zone(_zone)
                                        )
                                        _zhour = get_current_hour()
                                        _zsun = self._get_sun_times()
                                        _zbase_state = AreaState(
                                            is_circadian=True, is_on=True
                                        )
                                        _zbase_k = (
                                            CircadianLight.calculate_color_at_hour(
                                                _zhour,
                                                _zcfg,
                                                _zbase_state,
                                        ...
                                        _zbreakdown = (
                                            CircadianLight.get_solar_rule_breakdown(
                                                _zbase_k,
                                                _zhour,
                                                _zcfg,
                                                _zbase_state,
                                        ...
                                        glozone_state.set_zone_solar_cache(
                                            _zone,
                                            {
                                        ...
                                        logger.debug(
                                            f"Solar cache error for zone {_zone}: {_e}"
                                        )
                                logger.debug(
                ...
                if success:
                    ...
                    for gname, m in group_mapping.items():
                        ...
                        if ug:
                            area_name = m["area"]
                            normalized_name = area_name.lower().replace(" ", "_")
                            area_id = self.area_name_to_id.get(
