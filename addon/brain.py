...
def compute_override_decay(
    ...
    current_hour and next_phase_shift_hour are expected in h48 space.
    set_at is stored in regular 0-24 hour format, so t_ascend is used to
    convert it to h48 (adding 24 when the hour is before ascend_start,
    matching the convention in CircadianLight.get_phase_info).
    """
    if set_at is None:
        return 1.0
    # Convert set_at from regular hour to h48 space
    if t_ascend is not None and set_at < t_ascend:
        set_at = set_at + 24
...
def _converge_override(
    desired: float,
    base_cct: float,
    tolerance: float,
    render_fn,
) -> Optional[float]:
    """Find the color_override that makes render_fn produce desired CCT.

    Uses division-based correction: measures the effective attenuation from
    a first attempt, then divides it out for single-step convergence.
    Falls back to iterative refinement if the system is non-linear.

    Args:
        desired: Target rendered CCT
        base_cct: Rendered CCT with no override (solar rules applied)
        tolerance: Acceptable error in Kelvin
        render_fn: Callable(override) -> rendered CCT

    Returns:
        Override value, or None if no override needed.
    """
    if abs(base_cct - desired) <= tolerance:
        return None

    desired_shift = desired - base_cct
    override = desired_shift  # Initial estimate

    test_cct = render_fn(override)
    error = test_cct - desired
    if abs(error) <= tolerance:
        return override

    # Division-based correction: compute effective attenuation and divide it out
    actual_shift = test_cct - base_cct
    if abs(actual_shift) > 0.1 and abs(override) > 0.1:
        attenuation = actual_shift / override
        if 0.01 < abs(attenuation) < 10:
            corrected = desired_shift / attenuation
            verify_cct = render_fn(corrected)
            if abs(verify_cct - desired) < abs(error):
                override = corrected
                error = verify_cct - desired

    # If still not converged, iterate a few more times
    for _ in range(5):
        if abs(error) <= tolerance:
            break
        override -= error
...
class CircadianLight:
    ...
    # calculate_step deleted — stepping now uses calculate_set_position(dimension="step")
    ...
    def calculate_color_step(
        ...
        new_override = _converge_override(
            target, base_rendered, tolerance, _render_color
        )

        # 8. Verify final rendered output changed meaningfully
        actual_cct = _render_color(new_override)
    ...
    def calculate_set_position(
        ...
        if dimension == "step":
            ...
            if in_ascend:
                ...
                new_mid = (
                    CircadianLight.lift_midpoint_to_phase(
                        new_mid, t_descend, descend_end
                    )
                    % 24
                )
            state_updates["brightness_mid"] = new_mid
            state_updates["color_mid"] = new_mid
            # Clear step-originated color_override (no longer used for stepping)
        ...
        new_state = AreaState(
            ...
            brightness_override=state_updates.get(
                "brightness_override", state.brightness_override
            ),
            brightness_override_set_at=state_updates.get(
                "brightness_override_set_at", state.brightness_override_set_at
            ),
            color_override_set_at=state_updates.get(
                "color_override_set_at", state.color_override_set_at
            ),
        )
...
def calculate_dimming_step(
    ...
    **kwargs,
    ...
    """Calculate one step along the circadian curve from default position.

    Used by the light designer for step preview. Always starts from
    default state (no midpoint shift, no overrides).
    """
    from datetime import datetime
    ...
    # Current brightness at default position
    current_bri = CircadianLight.calculate_brightness_at_hour(hour, config, area_state)
    step_size = (config.max_brightness - config.min_brightness) / max_steps
    sign = 1 if action == "brighten" else -1
    target_bri = current_bri + sign * step_size
    ...
    # Convert to 0-100 position and use calculate_set_position
    ...
    result = CircadianLight.calculate_set_position(
        hour=hour, position=position, dimension="step",
        config=config, state=area_state,
    )
